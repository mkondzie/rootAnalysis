...
void readParameters(TF1 *fittingFunction[], int lDetector,
...
void calibrationSi() {
  ...
  const int nSpectra = *std::max_element(workingDetectors.begin(), workingDetectors.end()) + 1;
  std::cout << nSpectra << std::endl;
  // spectra
  TH1F *h[nSpectra];
  ...
  for (const auto &l : workingDetectors) {
    ...
    gr[l]->Fit(fChVeryk[l], "RMEN", "", 500, 4000);
    ...
    double energyAlphaMeV = 5.4395;////5.486

    K[l] = energyAlphaMeV / functionFormula(&xAlphaPeakChannel, calibrationParameters);
  }
...
// if (l >= sizeof(h)/sizeof(h[0])) {
//     std::cout << "Error: Invalid histogram index " << l << std::endl;
//     continue;
// }
...
//    canvasSpectra->cd(l);
//     h[l + 1]->Draw("Hist");
//     h[l + 1]->GetXaxis()->SetTitle("Channels");
//     h[l + 1]->SetTitle(Form("Spectra -  Silicon %i", l));
//     h[l + 1]->SetLineColor(kAzure + 2);
// 	  h[l + 1]->SetFillColor(kAzure + 2);
// 	  h[l + 1]->SetFillStyle(3002);
//     setPlottingOptions(h[l + 1]);}
// /////

// if (h[l]->GetEntries() == 0) {
//     std::cout << "Histogram " << l << " empty" << std::endl;
// } else {
//     std::cout << "Histogram  " << l << " filled" << std::endl;
// }
...
//    canvasAlpha->cd(l);
//     halpha[l]->Draw("Hist");
    // halpha[l]->GetXaxis()->SetTitle("Channels");
    // halpha[l]->SetTitle(Form("Spectra -  Silicon %i", l));
    // halpha[l]->GetXaxis()->SetRange(310,380);
// }
  // canvasSpectra->cd(12);
  // h[13]->Draw("Hist");
  // setPlottingOptions(h[13]);
